...
DATA_DIR = os.path.join(BASE_DIR,"data","Disease-Prediction-and-Medical dataset")
MODEL_DIR = os.path.join(BASE_DIR,"models","first_feature_models")
UTILS_DIR = os.path.join(BASE_DIR,"utils")
image_path = os.path.join(UTILS_DIR,"ph3_1.png")
...
st.sidebar.markdown(
    ...
    The Disease Prediction & Medicl Recommendation system uses AI to analyze symptoms, predict disease, assess health risks, and suggest personalized treatments—enhancing early diagnosis and improving healthcare decisions for better patient outcomes.
    </p>
    """,
    unsafe_allow_html=True
)
...
def load_data():
    try:
        sym_des = pd.read_csv(os.path.join(DATA_DIR, "symptoms_df.csv"))
        ...
        precautions = precautions.loc[:, ~precautions.columns.str.contains("^Unnamed")]
        workout = workout.loc[:, ~workout.columns.str.contains("^Unnamed")]

        with open(os.path.join(MODEL_DIR, "RandomForest.pkl"), "rb") as f:
            model = pickle.load(f)
...
if st.button("Predict Disease"):
    if user_input:
        ...
        if patient_symptoms:
            ...
            for i, item in enumerate(workout, 1):
                if item:
                    st.write(f"{i}. {item}")

            st.subheader("🥗 Recommended Diet")
            for i, item in enumerate(rec_diet, 1):
                if item:
                    st.write(f"{i}. {item}")
                else:
